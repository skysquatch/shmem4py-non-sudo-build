...
[DONE]  Environment block written to ~/.bashrc
...
**`~/.bashrc`** — One block is appended between the markers:
```
...
The block sets `PATH`, `LD_LIBRARY_PATH`, `PKG_CONFIG_PATH`, `LDFLAGS`,
`CPPFLAGS`, OpenSHMEM variables (`SHMEM_HOME`, `OSHCC`, `UCX_TLS`, etc.),
and activates the Python venv automatically on login. The block is written
only once — re-running the script will not add it a second time.
...
```bash
rm -rf ~/local ~/shmem-build ~/shmem-venv ~/shmem-install.log
```

Then remove the block from `~/.bashrc` — delete the lines between and
